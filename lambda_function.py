...
def get_bird_data(x_lng, x_lat, x_days, x_dist):
    ...
    geo_url = 'https://ebird.org/ws2.0/data/obs/geo/recent?lng=' + x_lng + '&lat=' + x_lat + '&dist=' + x_dist + '&back=' + x_days + '&maxResults=1000&fmt=json&includeProvisional=true&key=' + ebird_key
    

    try:
        data = json.load(urllib2.urlopen(geo_url))
    ...
    for i in data:
        ...
        bird_list.append([str(i.get('comName')), str(i.get('howMany')), new_date + '  (' + str(days.get(dow)) + ')' ,str(i.get('locName'))])
